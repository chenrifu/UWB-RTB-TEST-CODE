...
void rx_listener_cb(const dwt_cb_data_t *rxd)
{
    ...
    if(CIRC_SPACE(head,tail,size) > 0)
    {
        ...
        dwt_readrxtimestamp(p->timeStamp);      //Raw Rx TimeStamp (STS or IPATOV based on STS config)
        ...
        if(app.listenerTask.Handle)         // RTOS : listenerTask can be not started yet
        {
            head = (head + 1) & (size-1);
    ...
    if(app.listenerTask.Handle)         // RTOS : listenerTask can be not started yet
    {
        ...
        if(osSignalSet(app.listenerTask.Handle, app.listenerTask.Signal) != osOK)
        {
    ...
    dwt_readeventcounters(&app.event_counts); //take a snapshot of event counters
...
error_e listener_process_init()
{
     ...
     *
     * */
    pListenerInfo->pSfConfig = &app.pConfig->s.sfConfig; /**< Super Frame configuration */
    ...
    dwt_setcallbacks(NULL, rx_listener_cb, listener_timeout_cb, listener_error_cb, NULL, NULL, NULL);

 ...
 * In case Tag's crystal is very off, it also will be trimmed to stay in the |5 .. 6| ppm range,
 * as defined below:
 * */
#define TARGET_XTAL_OFFSET_VALUE_PPHM_MIN   (500)
#define TARGET_XTAL_OFFSET_VALUE_PPHM_MAX   (800)
/* The typical trimming range of DW3000 shield (with 2pF external caps is ~48ppm (-30ppm to +18ppm) over all steps */
...
void trim_tag_proc(tag_info_t *pTwrInfo)
{
       ...
       tmp < TARGET_XTAL_OFFSET_VALUE_PPHM_MIN)
    {
        ...
        tmp8 -= (int8_t)(((TARGET_XTAL_OFFSET_VALUE_PPHM_MAX + TARGET_XTAL_OFFSET_VALUE_PPHM_MIN)/2 + pTwrInfo->clkOffset_pphm) * AVG_TRIM_PER_PPHM);//(TARGET_XTAL_OFFSET_VALUE_PPHM_MAX + TARGET_XTAL_OFFSET_VALUE_PPHM_MIN)/2
...
twr_tx_tag_cb(const dwt_cb_data_t *txd)
{
    ...
    uint32_t tmp =  nrf_drv_rtc_counter_get(&rtc);;                    // NRF: MCU RTC time : this will be HW timestamped
...
twr_rx_cb(const dwt_cb_data_t *rxd)
{
    ...
    p->rtcTimeStamp =  nrf_drv_rtc_counter_get(&rtc);                     // NRF MCU RTC timestamp

    {
        dwt_readrxtimestamp(p->timeStamp) ; //Rx TimeStamp from Ipatov CIR when STS OFF or from STS when STS ON
...
error_e tag_process_init(void)
{
    ...
    {// For Trilat demo purposes Fixed tags can supply their locations in the payload
    ...
    dwt_setcallbacks(twr_tx_tag_cb, twr_rx_cb, twr_rx_timeout_cb, twr_rx_error_cb, NULL, tag_spi_rdy_cb, NULL);

    dwt_setinterrupt( DWT_INT_SPIRDY_BIT_MASK | DWT_INT_TXFRS_BIT_MASK | DWT_INT_RXFCG_BIT_MASK
    ...
    pTagInfo->env.responseRxTo_sy  = 4 * MAX(pTagInfo->msg_time.response.sy,pTagInfo->msg_time.ranging_config.sy); //TODO: fix "4". to be able receive long rng_cfg_upd_msg_t relax Response RX timeout time
    ...
    if((dwt_getxtaltrim() == DEFAULT_XTAL_TRIM) || (app.pConfig->s.xtalTrim & ~XTAL_TRIM_BIT_MASK))
    {
        ...
        /* configure the RTC Wakeup timer with a RTOS priority:
         * tag need to send signals to tasks
         * */
        NVIC_SetPriority(RTC0_IRQn, PRIO_RTC_WKUP_IRQn);

        juniper_configure_hal_rtc_callback(rtcWakeUpTimerEventCallback_tag);

        tag_configure_rtc_wakeup_ns(10*1e6);  //first blink will start in 10ms after starting of the Tag application
...
error_e    initiator_send_blink(tag_info_t *p)
{
    ...
    txPckt.delayedRxTime_sy     = (uint32_t)util_us_to_sy(app.pConfig->s.rcDelay_us);  //Ranging Config: activate receiver this time SY after Blink Tx
    txPckt.delayedRxTimeout_sy  = (uint32_t)util_us_to_sy(app.pConfig->s.rcRxTo_us);   //Ranging Config: receiver will be active for this time, SY
...
error_e    initiator_send_poll(tag_info_t *p)
{
    ...
    txPckt.delayedRxTime_sy     = p->env.delayRx_sy;        //environment, received from Ranging Config message
...
error_e tag_wakeup_dw3000_blink_poll(tag_info_t *p)
{
    ...
    {
        ret = port_wakeup_dw3000_fast(); //should never return anything other than _NO_ERR
    }
    else
    {
...
error_e twr_initiator_algorithm_rx(rx_pckt_t_t *pRxPckt, tag_info_t *pTwrInfo)
{
    ...
    if(pMsg->mac.frameCtrl[0] == Head_Msg_STD || pMsg->mac.frameCtrl[0] == Head_Msg_STD_AR)
    {
        /* Only SS and LS MAC headers supported in current application */
        switch (pMsg->mac.frameCtrl[1] & Frame_Ctrl_MASK)
        {
        case Frame_Ctrl_SS:
            if((pRxPckt->rxDataLen == sizeof(resp_pdoa_msg_t)) ||    /* Response (with Coordinates extension) */
    ...
    switch (fcode)
    {
        case Twr_Fcode_Rng_Config :
            ...
            {
                ret = _NO_Err_Can_Sleep; //all good and wrong RX except Update would lead to dwt_entersleep()
            }
            break;
        ...
        case Twr_Fcode_Resp_Ext:
             ...
             * */
            if (pTwrInfo->env.nodeAddr[0] == ((resp_pdoa_msg_t*)pMsg)->mac.sourceAddr[0] ||
                pTwrInfo->env.nodeAddr[1] == ((resp_pdoa_msg_t*)pMsg)->mac.sourceAddr[1])
            {
                ...
                pTwrInfo->env.slotCorr_ns = 1000 * (int32_t)AR2U32(((resp_pdoa_msg_t*)pMsg)->resp.slotCorr_us);

                slotCorr_ns = pTwrInfo->env.slotCorr_ns;

                //(A)start of calculation
                rtcNow = nrf_drv_rtc_counter_get(&rtc);;                    // NRF: MCU RTC time : this will be HW timestamped
 ...
 * */
error_e initiator_received_ranging_config(rx_pckt_t_t *pRxPckt, tag_info_t *pTwrInfo)
{
    ...
    /*check units*/
    if((pRxMsg->version == RC_VERSION_DR) && (pRxPckt->msg.rngCfgMsg.rngCfg.ANC_RESP_DLY[1] & 0x80)) //units are ms
    {
        units_ms = 1;
        ret = _NO_ERR;
    }
    ...
    if(ret == _NO_ERR)
    {
        ret = (pRxPckt->rxDataLen == sizeof(rng_cfg_msg_t))?(_NO_Err_Ranging_Config):(_NO_Err_Ranging_Update);
        ...
        if(units_ms == 1)
        {
            uint32_t ancResp = (AR2U16(pRxPckt->msg.rngCfgMsg.rngCfg.ANC_RESP_DLY)&0x7FFF);
            uint32_t tagResp = (AR2U16(pRxPckt->msg.rngCfgMsg.rngCfg.TAG_RESP_DLY)&0x7FFF);

            pTwrInfo->env.pollTx2FinalTxDelay64 = util_us_to_dev_time((ancResp + tagResp) * 1000); //Time from RMARKER of Poll to RMARKER of Final.
            //pTwrInfo->env.delayRx_sy            = (uint32_t)( (((uint32_t)AR2U16(pRxMsg->delayRx_us))<<16) + AR2U16(pRxMsg->pollTxToFinalTx_us) );      //RX turn on delay following Poll transmission
            pTwrInfo->env.delayRx_sy            = (uint32_t)util_us_to_sy(((AR2U16(pRxPckt->msg.rngCfgMsg.rngCfg.ANC_RESP_DLY)&0x7FFF))*1000 - pTwrInfo->msg_time.poll.us);      //RX turn on delay following Poll transmission
        }
        else
        {
            pTwrInfo->env.pollTx2FinalTxDelay64 = util_us_to_dev_time(AR2U16(pRxMsg->pollTxToFinalTx_us)); //Time from RMARKER of Poll to RMARKER of Final.
        ...
        tag_configure_rtc_wakeup_ns(tmp);  // update the RTC Wakup timer to start Ranging in assigned slot

        NVIC_EnableIRQ(RTC0_IRQn);//NRF
...
error_e initiator_received_response(rx_pckt_t_t *prxPckt, tag_info_t *p)
{
    ...
    calcFinalTx64 = MASK_TXDTS & ((uint64_t)(calcFinalTx64 + p->env.pollTx2FinalTxDelay64));
    ...
    calcFinalTx64 = MASK_40BIT & (uint64_t)(calcFinalTx64 + app.pConfig->s.antTx_a);
    ...
    TS2TS_UWB_MEMCPY( pTxMsg->final.responseRx_ts, prxPckt->timeStamp);   // Embed Response Rx time to the Final message
...
void tag_configure_rtc_wakeup_ns(uint32_t     period_ns)
{
    ...
    gRTC_SF_PERIOD = period_ns / WKUP_RESOLUTION_NS;
...
void rtcWakeUpTimerEventCallback_tag(void)
{
    ...
    old_cc = nrf_drv_rtc_counter_get(&rtc);
    new_cc = gRTC_SF_PERIOD; //this can be any value in RTC time units, e.g. default period

    if(pTwrInfo)    //RTOS : pTwrInfo can be not allocated yet
    {
      pTwrInfo->gRtcSFrameZeroCnt = old_cc;

      if(pTwrInfo->mode == BLINKING_MODE)
      {//BLINKING phase
          ...
          new_cc = (uint32_t)((float)((BLINK_PERIOD_MS/3.0)*rand()/RAND_MAX));
          new_cc += BLINK_PERIOD_MS;
          new_cc *= 1e3;
          new_cc /= WKUP_RESOLUTION_US;
      ...
      {//RANGING phase
          new_cc = pTwrInfo->env.sframePeriod_ns; //After first Slot_correction set the 
          new_cc /= WKUP_RESOLUTION_NS;           //period to the defined sframePeriod_ns
